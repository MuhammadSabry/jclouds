 ...
 * @author Adrian Cole
 */
@Test(groups = "unit", testName = "CloudFilesExpectTest")
public class CloudFilesExpectTest extends BaseCloudFilesRestClientExpectTest {
   ...
   public void deleteImageReturnsTrueOn200AndFalseOn404() {
      ...
      HttpRequest deleteImage11 = HttpRequest.builder().method("DELETE").endpoint(
               URI.create("https://files.api.rackspacecloud.com/v1.0/123123/images/11?now=1257695648897")).headers(
               ImmutableMultimap.<String, String> builder()
               .put("X-Auth-Token", authToken).build()).build();
      
      HttpResponse imageDeleted = HttpResponse.builder().statusCode(204).message("HTTP/1.1 204 No Content").build();

      CloudFilesClient clientWhenImageExists = requestsSendResponses(initialAuth, responseWithAuth, deleteImage11, imageDeleted);
//      assert clientWhenImageExists.deleteImage(11);
      ...
      CloudFilesClient clientWhenImageDoesntExist =  requestsSendResponses(initialAuth, responseWithAuth, deleteImage11, imageNotFound);
